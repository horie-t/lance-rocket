...
class XilinxNexys4DDRMIGIsland(c : XilinxNexys4DDRMIGParams)(implicit p: Parameters) extends LazyModule with HasCrossing {
...
class XilinxNexys4DDRMIG(c : XilinxNexys4DDRMIGParams)(implicit p: Parameters) extends LazyModule {
  val ranges = AddressRange.fromSets(c.address)
  val depth = ranges.head.size
